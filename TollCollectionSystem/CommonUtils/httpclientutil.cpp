 ...
 */
enum class HttpClientRequestMethod { GET, POST, PUT, DELETE, UPLOAD };
...
/**
 * @brief HttpClientUtil 的辅助类，封装不希望暴露给客户端的数据和方法，使得 HttpClientUtil 只暴露必要的 API 给客户端。
 */
class HttpClientPrivate {
    friend class HttpClientUtil;
     ...
     */
    static void handleFinish(HttpClientPrivateCache cache, QNetworkReply *reply, const QString &successMessage,
    ...
    std::function<void(const QString &, int)> failHandler =
        nullptr;  // 失败的回调函数，参数为失败原因和 HTTP status code
...
QNetworkAccessManager *HttpClientPrivate::getManager() { return internal ? new QNetworkAccessManager() : manager; }
...
void HttpClientPrivate::executeQuery(HttpClientPrivate *d, HttpClientRequestMethod method) {
    ...
    switch (method) {
        ...
        case HttpClientRequestMethod::POST:
            reply = cache.manager->post(
                request, d->useJson ? d->json.toUtf8() : d->params.toString(QUrl::FullyEncoded).toUtf8());
            break;
        case HttpClientRequestMethod::PUT:
            reply = cache.manager->put(request,
                                       d->useJson ? d->json.toUtf8() : d->params.toString(QUrl::FullyEncoded).toUtf8());
            break;
...
void HttpClientPrivate::upload(HttpClientPrivate *d, const QStringList &paths, const QByteArray &data) {
    ...
    if (paths.size() > 0) {
        ...
        for (const QString &path : paths) {
            ...
            QString disposition =
                QString("form-data; name=\"%1\"; filename=\"%2\"").arg(inputName).arg(file->fileName());
            QHttpPart filePart;
    ...
    QObject::connect(reply, &QNetworkReply::finished, [=] {
        ...
        QString successMessage =
            HttpClientPrivate::readReply(reply, cache.charset);  // 请求结束时一次性读取所有响应数据
        QString failMessage = reply->errorString();
        HttpClientPrivate::handleFinish(cache, reply, successMessage, failMessage);
    });
}
...
void HttpClientPrivate::download(HttpClientPrivate *d, const QString &savePath) {
    ...
    // [3] 调用下载的重载函数开始下载
    HttpClientPrivate::download(d, [=](const QByteArray &data) { file->write(data); });
}
...
// 请求结束的处理函数
void HttpClientPrivate::handleFinish(HttpClientPrivateCache cache, QNetworkReply *reply, const QString &successMessage,
                                     const QString &failMessage) {
    // 1. 执行请求成功的回调函数
    // 2. 执行请求失败的回调函数
    // 3. 执行请求结束的回调函数
...
HttpClientUtil::HttpClientUtil(const QString &url) : d(new HttpClientPrivate(url)) {}

HttpClientUtil::~HttpClientUtil() { delete d; }

// 传入 QNetworkAccessManager 给多个请求共享
HttpClientUtil &HttpClientUtil::manager(QNetworkAccessManager *manager) {
    d->manager  = manager;
...
HttpClientUtil &HttpClientUtil::debug(bool debug) {
    d->debug = debug;
...
HttpClientUtil &HttpClientUtil::param(const QString &name, const QVariant &value) {
...
// 添加多个请求的参数
HttpClientUtil &HttpClientUtil::params(const QMap<QString, QVariant> &ps) {
...
HttpClientUtil &HttpClientUtil::json(const QString &json) {
    d->json    = json;
    d->useJson = true;
...
// 添加请求头
HttpClientUtil &HttpClientUtil::header(const QString &name, const QString &value) {
...
// 添加多个请求头
HttpClientUtil &HttpClientUtil::headers(const QMap<QString, QString> nameValues) {
...
// 注册请求成功的回调函数
HttpClientUtil &HttpClientUtil::success(std::function<void(const QString &)> successHandler) {
...
// 注册请求失败的回调函数
HttpClientUtil &HttpClientUtil::fail(std::function<void(const QString &, int)> failHandler) {
...
HttpClientUtil &HttpClientUtil::complete(std::function<void()> completeHandler) {
...
// 设置请求响应的编码
HttpClientUtil &HttpClientUtil::charset(const QString &cs) {
    d->charset = cs;
...
// 执行 GET 请求
void HttpClientUtil::get() { HttpClientPrivate::executeQuery(d, HttpClientRequestMethod::GET); }

// 执行 POST 请求
void HttpClientUtil::post() { HttpClientPrivate::executeQuery(d, HttpClientRequestMethod::POST); }

// 执行 PUT 请求
void HttpClientUtil::put() { HttpClientPrivate::executeQuery(d, HttpClientRequestMethod::PUT); }

// 执行 DELETE 请求
void HttpClientUtil::remove() { HttpClientPrivate::executeQuery(d, HttpClientRequestMethod::DELETE); }

// 使用 GET 进行下载，下载的文件保存到 savePath
void HttpClientUtil::download(const QString &savePath) { HttpClientPrivate::download(d, savePath); }

// 上传文件
void HttpClientUtil::upload(const QString &path) {
    QStringList paths = {path};
    HttpClientPrivate::upload(d, paths, QByteArray());
}

// 上传文件，文件的内容以及读取到 data 中
void HttpClientUtil::upload(const QByteArray &data) { HttpClientPrivate::upload(d, QStringList(), data); }

// 上传多个文件
void HttpClientUtil::upload(const QStringList &paths) { HttpClientPrivate::upload(d, paths, QByteArray()); }
