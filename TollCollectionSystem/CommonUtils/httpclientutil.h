#ifndef HTTPCLIENTUTIL_H
#define HTTPCLIENTUTIL_H
 ...
 *
 * 默认 HttpClientUtil 会创建一个 QNetworkAccessManager，如果不想使用默认的，调用 manager() 传入即可。
 * 调用 debug(true) 设置为调试模式，输出调试信息如 URL、参数等。
 */

class HttpClientUtil {
public:
    HttpClientUtil(const QString &url);
    ~HttpClientUtil();

    /**
     * @brief 每创建一个 QNetworkAccessManager 对象都会创建一个线程，当频繁的访问网络时，为了节省线程资源，
     *     可以传入 QNetworkAccessManager 给多个请求共享 (它不会被 HttpClientUtil 删除，用户需要自己手动删除)。
     *     如果没有使用 manager() 传入一个 QNetworkAccessManager，则 HttpClientUtil
     * 会自动的创建一个，并且在网络访问完成后自动删除它。
     *
     * @param  manager 执行 HTTP 请求的 QNetworkAccessManager 对象
     * @return 返回 HttpClientUtil 的引用，可以用于链式调用
     */
    HttpClientUtil &manager(QNetworkAccessManager *manager);
     ...
     * @return 返回 HttpClientUtil 的引用，可以用于链式调用
     */
    HttpClientUtil &debug(bool debug);
     ...
     * @return 返回 HttpClientUtil 的引用，可以用于链式调用
     */
    HttpClientUtil &param(const QString &name, const QVariant &value);
     ...
     * @return 返回 HttpClientUtil 的引用，可以用于链式调用
     */
    HttpClientUtil &params(const QMap<QString, QVariant> &ps);
     ...
     * @return 返回 HttpClientUtil 的引用，可以用于链式调用
     */
    HttpClientUtil &json(const QString &json);
     ...
     * @return 返回 HttpClientUtil 的引用，可以用于链式调用
     */
    HttpClientUtil &header(const QString &name, const QString &value);
     ...
     * @return 返回 HttpClientUtil 的引用，可以用于链式调用
     */
    HttpClientUtil &headers(const QMap<QString, QString> nameValues);
     ...
     * @return 返回 HttpClientUtil 的引用，可以用于链式调用
     */
    HttpClientUtil &success(std::function<void(const QString &)> successHandler);
     ...
     * @return 返回 HttpClientUtil 的引用，可以用于链式调用
     */
    HttpClientUtil &fail(std::function<void(const QString &, int)> failHandler);
     ...
     * @return 返回 HttpClientUtil 的引用，可以用于链式调用
     */
    HttpClientUtil &complete(std::function<void()> completeHandler);
     ...
     * @return 返回 HttpClientUtil 的引用，可以用于链式调用
     */
    HttpClientUtil &charset(const QString &cs);
...
private:
    HttpClientPrivate *d = nullptr;
};

#endif  // HTTPCLIENTUTIL_H
